...
styled through the `--nx-*` token namespace.
...
```typescript
// Wiring the prose token namespace for the nx demo
const proseTokens = {
  vspMd:        `var(--nx-vsp-md)`,
  textBody:     `var(--nx-text-body)`,
  leadingRelaxed: `var(--nx-leading-relaxed)`,
  codeBg:       `var(--nx-code-bg)`,
  codeFg:       `var(--nx-code-fg)`,
  fontMono:     `var(--nx-font-mono)`,
} as const;
...
1. Define the token set in `--nx-*` namespace.
...
- **Next.js** — `/` (the configured `basePath`)
...
Heading borders and blockquote accents reuse the shared `--nx-color-primary`,
`--nx-color-muted`, and `--nx-color-accent` tokens — no additional colour tokens
are required.
